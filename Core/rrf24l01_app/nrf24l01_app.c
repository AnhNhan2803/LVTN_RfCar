...
#else
#define NRF24L01_RX_THREAD_NAME ("nrf24l01_rx_thread")
#define NRF24L01_RX_THREAD_STACK_SIZE (1024) // Bytes
#define NRF24L01_RX_THREAD_PRIORITY ((osPriority_t)osPriorityHigh1)
...
// Create threading parameters for the Receiver
static const osThreadAttr_t nrf24l01_rx_thread_attr =
    {
        .name = NRF24L01_RX_THREAD_NAME,
        .priority = NRF24L01_RX_THREAD_PRIORITY,
        .stack_size = NRF24L01_RX_THREAD_STACK_SIZE,
};
static __attribute__((unused)) osThreadId_t nrf24l01_rx_thread_handle;
#endif
...
static void nrf24l01_rx_thread_entry(void *argument);
...
void nrf24l01_thread_app_init(void)
{
  ...
  nrf24l01_rx_thread_handle = osThreadNew(nrf24l01_rx_thread_entry, NULL, &nrf24l01_rx_thread_attr);
  if (nrf24l01_rx_thread_handle != NULL)
  {
    PRINT_INFO_LOG("Successfully create the NRF24L01 RX thread!\r\n");
  }
#endif
}
...
#if (DEVICE_ROLE == DEVICE_ROLE_TX)
/******************************************************************************
* Function : static void nrf24l01_rx_thread_entry (void *argument)
* Brief    : Thread entry handles all RX operations of NRF24L01.
* Input    : None.
* Output   : None.
* Return   : None.
*******************************************************************************/
    ...
    while(1)
    {
      if(nrf24l01_get_data_from_tx_queue(tx_data))
      {
        if((tx_data[0] == RF_CMD_HEADER1) && (tx_data[1] == RF_CMD_HEADER2))
        {
          // switch(tx_data[3])
          // {
          //   case RF_CMD_GET_AVAILABLE_SSID:
          ...
          //   case RF_CMD_GET_RSSI:
          //     break;

          //   default:
          //     // Only transmit packet here
...
static void nrf24l01_tx_send_in_order_and_get_response(uint8_t * tx_data, uint8_t tx_len, uint8_t * rx_data, uint8_t * rx_len)
{
  ...
  for(uint8_t idx=0; idx<NRF24L01_MAX_NUM_SEND_ORDER; idx++)
  {
    ...
    else
    {
      ...
      if (status != 0x01)
      {
        // Cache the data received fro MCU
